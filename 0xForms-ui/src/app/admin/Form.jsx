...
export function QuestionCard() {
    ...
    let removeField = (id)=>{
        ...
        let newQuestions = questions.filter((e)=>e.id != id);
        setQuestions([...newQuestions]);
    }

    let handleInputChange = (e, fieldOfField, index)=>{
    ...
    let createOptions = (e, length, index)=>{
        let tempQuestions = [...questions];
        if(e.code == "n"){
    ...
    let handleRadioCheckboxLable= (e, questionIndex, oIndex) =>{
        let tempQuestions = [...questions];
                    ...
                    <div key={index} className="q-card question flex flex-col justify-center items-center gap-5">
                        ...
                        <div className="q-n-type flex flex-col ">
                                ...
                                <div className="flex flex-row items-center justify-center gap-5">
                                    ...
                                    {/* <button onClick={(e)=> removeField(index)} >remove</button> */}
                                </div>
                        </div>
                                                ...
                                                {   
                                                    field["options"].map((_q, oIndex)=>(
                                                        <div key={oIndex} className="inputs flex gap-3">
                                                            ...
                                                            <input type='text' className={field["questionType"]+"-lable border-b input-padding"} placeholder={"option "+(oIndex+1)} onChange={(e)=>handleRadioCheckboxLable(e, index, oIndex)} onKeyDown={(e)=>createOptions(e, field.options.length+1, index)} />
                                                        </div>
                                                    ))
                                                }
                                                </div>
                        ...
                        <div className="toolbar flex flex-row justify-start items-center w-full mt-3 ml-2">
                            ...
                            <div onClick={(e)=> removeField(index)} className="delete-btn p-1 hover:bg-slate-500 transition ease-in-out delay-15 rounded-full">
